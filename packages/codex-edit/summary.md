...
A future go decision should require a larger workload dominated by coordinated multi-file edits and must show a measurable improvement in total task completion time or tool-call count without lowering exact correctness. The current `edit` remains the default.
