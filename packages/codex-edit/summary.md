...
The production route was validated with `pudu-openai-proxy/gpt-5.6-sol:high` and `pudu-openai-proxy/gpt-5.6-luna:high`. Sol's 20-case fair run improved first editor-call success from 85% to 95% with equal final correctness. For Luna, counting only normally completed runs whose final file tree is correct gives `edit` 15/20 with a 24.0s median and `apply_patch` 11/20 with a 49.9s median; eight Luna apply runs reached the 180s harness limit or otherwise remained incomplete. Among the 10 cases correct in both arms, apply_patch was 26.9s slower at the median. A small four-case interleaved crossover was faster for apply_patch in three cases, so proxy variance exists, but the full run still does not support Luna enablement. The default package allowlist therefore enables Sol only; Luna and Terra require explicit opt-in validation.
